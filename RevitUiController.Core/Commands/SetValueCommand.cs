...
public class SetValueCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (value == null)
            {
                Console.Write(OutputFormatter.FormatError("PatternNotSupported", name, ["ValuePattern not available, try type"], CoreSettings.GlobalOptions));
                return Task.FromResult(1);
            }
            value.SetValue(text);
        ...
        {
            Console.Write(OutputFormatter.FormatError("SetValueFailed", name, [ex.Message], CoreSettings.GlobalOptions));
            return Task.FromResult(1);
        }
    }
}
