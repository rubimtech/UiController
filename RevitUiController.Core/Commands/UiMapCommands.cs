...
public class UiMapLoadCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        ...
        if (!loaded)
        {
            Console.Write(OutputFormatter.FormatError("LoadFailed", path, null, CoreSettings.GlobalOptions));
...
public class UiMapSaveCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        ...
        {
            Console.Write(OutputFormatter.FormatError("SaveFailed", ex.Message, null, CoreSettings.GlobalOptions));
            return Task.FromResult(1);
        }
    }
}
...
public class UiMapResolveCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        if (args.Length == 0)
        {
            Console.Write(OutputFormatter.FormatError("InvalidArgs", Usage, null, CoreSettings.GlobalOptions));
            return Task.FromResult(1);
        }
        ...
        if (candidates.Count == 0)
        {
            ...
            Console.Write(OutputFormatter.FormatError("NotFound", logicalName, suggestions, CoreSettings.GlobalOptions));
            return Task.FromResult(1);
        }
...
public class UiMapRegisterCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        ...
        {
            Console.Write(OutputFormatter.FormatError("InvalidArgs", "logical-name is required", null, CoreSettings.GlobalOptions));
            return Task.FromResult(1);
        }
...
public class UiMapListCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        if (!UiMap.IsLoaded)
        {
            Console.Write(OutputFormatter.FormatError("NotLoaded", "no uimap loaded", null, CoreSettings.GlobalOptions));
            return Task.FromResult(1);
        }
...
public class UiMapAutoCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        ...
        var found = AutomationHelper.FindFirstEnabledVisible(window, elementName);
        if (found == null)
        {
            Console.Write(OutputFormatter.FormatError("NotFound", elementName, null, CoreSettings.GlobalOptions));
            return Task.FromResult(1);
        }
