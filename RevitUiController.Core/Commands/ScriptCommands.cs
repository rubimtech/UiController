...
public class WaitCommand : ICommand
{
    ...
    public async Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        ...
        {
            Console.Write(OutputFormatter.FormatError("InvalidArgs", "wait <seconds>", null, CoreSettings.GlobalOptions));
            return 1;
        }
...
public class ScriptCommand : ICommand
{
    ...
    public async Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        ...
        if (fileArgs.Count == 0)
        {
            Console.Write(OutputFormatter.FormatError("InvalidArgs", "script <file-path> [--auto-heal] [--auto-heal-max-retry N]", null, CoreSettings.GlobalOptions));
            return 1;
        }
        ...
        {
            Console.Write(OutputFormatter.FormatError("FileNotFound", filePath, null, CoreSettings.GlobalOptions));
            return 1;
        }
        ...
        foreach (var rawLine in lines)
        {
            ...
            if (cmdName == "wait-for" && cmdArgs.Length > 0)
            {
                ...
                var dialog = await Retry.WaitForDialog(window, title, timeout, ct: ct);
                if (dialog != null)
                {
            ...
            if (cmdName == "wait-close" && cmdArgs.Length > 0)
            {
                ...
                var closed = await Retry.WaitForDialogClose(window, title, timeout, ct: ct);
                if (closed)
                {
            ...
            executed++;
            var exitCode = await cmd.ExecuteAsync(window, cmdArgs, ct);
            ...
            if (exitCode != 0 && autoHeal)
            {
                for (int healAttempt = 0; healAttempt < autoHealMaxRetry; healAttempt++)
                {
                    ...
                    try { await SafetyGuard.DismissWarningDialogs(window, ct); } catch { }
                    ...
                    exitCode = await cmd.ExecuteAsync(window, cmdArgs, ct);
                    if (exitCode == 0)
                    {
