...
public class ClipboardGetCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        return Task.Run(() =>
        {
            ...
            {
                thread.Interrupt();
                Console.Write(OutputFormatter.FormatError("ClipboardError", "read", ["Clipboard access timed out"], CoreSettings.GlobalOptions));
                return 1;
            }
...
public class ClipboardSetCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        ...
        return Task.Run(() =>
        {
            ...
            {
                thread.Interrupt();
                Console.Write(OutputFormatter.FormatError("ClipboardError", "write", ["Clipboard access timed out"], CoreSettings.GlobalOptions));
                return 1;
            }
            return success ? 0 : 1;
        }, ct);
