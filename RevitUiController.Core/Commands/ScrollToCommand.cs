...
public class ScrollToCommand : ICommand
{
    ...
    public async Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        ...
        {
            scope = AutomationHelper.FindFirstEnabledVisible(window, parentFilter);
            if (scope == null)
            {
                Console.Write(OutputFormatter.FormatError("NotFound", parentFilter, ["parent"], CoreSettings.GlobalOptions));
                return 1;
            }
        }
