    ...
    public async Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        ...
        if (result == null)
        {
            Console.Write(OutputFormatter.FormatResult(new CommandResult
            {
                ...
                Screenshot = CoreSettings.IsScreenshot ? base64 : null,
                DurationMs = elapsed
            }, CoreSettings.GlobalOptions));
            return 1;
        }
    ...
    public async Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        ...
        if (findResult == null)
        {
            Console.Write(OutputFormatter.FormatResult(new CommandResult
            {
                ...
                Screenshot = CoreSettings.IsScreenshot ? base64Before : null,
                DurationMs = elapsed
            }, CoreSettings.GlobalOptions));
            return 1;
        }
        ...
        var beforeState = OutputFormatter.CaptureState(window);

        await MouseControl.ClickAt(elementX, elementY, ct);
        await Task.Delay(300, ct);

        var afterState = OutputFormatter.CaptureState(window);
        ...
        Console.Write(OutputFormatter.FormatResult(new CommandResult
        {
            ...
            Diff = diff,
            Screenshot = CoreSettings.IsScreenshot ? base64After : null,
