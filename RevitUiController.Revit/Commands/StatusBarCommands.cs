...
public class StatusBarCommand : ICommand
{
    ...
    {
        var text = ReadStatusBarText(window);
    ...
    private static string ReadStatusBarText(AutomationElement window)
    {
        try
        {
            var stack = new Queue<AutomationElement>();
            stack.Enqueue(window);
            while (stack.Count > 0)
            {
        ...
        try
        {
            foreach (var c in SafeGetChildren(window, 10000))
            {
                try
                {
...
public class WaitProgressCommand : ICommand
{
    ...
    public async Task<int> ExecuteAsync(AutomationElement window, string[] args, CancellationToken ct = default)
    {
        ...
        {
            var completed = await CoreSettings.EventService.WaitForProgressAsync(timeout, ct);
            Console.Write(OutputFormatter.FormatResult(new CommandResult
            {
        ...
        while (DateTime.UtcNow < deadline)
        {
            try
            {
                var progress = FindProgressBar(window);
                if (progress == null)
                {
                    if (progressDetected)
                    {
