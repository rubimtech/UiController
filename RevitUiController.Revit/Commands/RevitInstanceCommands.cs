...
public class RevitInstancesCommand : UiCommandBase
{
    ...
    protected override Task<CommandResult> ExecuteInternalAsync(AutomationElement window, string[] args, CancellationToken ct)
    {
        var manager = RevitContext.InstanceManager;
...
public class RevitLaunchCommand : UiCommandBase
{
    ...
    protected override async Task<CommandResult> ExecuteInternalAsync(AutomationElement window, string[] args, CancellationToken ct)
    {
        int? year = null;
