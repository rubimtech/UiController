...
public class ListWindowsCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement revitWindow, string[] args)
    {
        ...
        var result = new CommandResult
        {
            ...
            Success = true,
            Data = Program.Verbosity == "minimal" ? null : new { mainWindow = revitWindow.Name, windows = elements, count = elements.Count }
        };
...
public class ListControlsCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement revitWindow, string[] args)
    {
        ...
        var maxDepth = Program.Verbosity == "full" ? 5 : Program.Verbosity == "normal" ? 3 : 1;
        ...
        var result = new CommandResult
        {
            ...
            Success = true,
            Data = Program.Verbosity == "minimal" ? null : new { windows = elements, count = elements.Count, filter }
        };
        if (Program.IsScreenshot)
            result.Screenshot = ScreenshotHelper.CaptureWindow(revitWindow);
        Console.Write(OutputFormatter.FormatResult(result, Program.IsPretty));
...
public class InfoCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement revitWindow, string[] args)
    {
        ...
        var result = new CommandResult
        {
            ...
            Success = true,
            Data = Program.Verbosity == "minimal"
                ? new { name = revitWindow.Name, controlType = revitWindow.ControlType.ToString() }
                : new
                {
                    name = revitWindow.Name,
                    bounds = new { x = rect.X, y = rect.Y, width = rect.Width, height = rect.Height },
                    isEnabled = revitWindow.IsEnabled,
                    isOffscreen = revitWindow.IsOffscreen,
                    controlType = revitWindow.ControlType.ToString(),
                    automationId = revitWindow.AutomationId ?? ""
                }
        };
        if (Program.IsScreenshot)
            result.Screenshot = ScreenshotHelper.CaptureWindow(revitWindow);
...
public class DumpCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement revitWindow, string[] args)
    {
        ...
        if (outputFile != null)
        {
            ...
            sb.AppendLine($"Revit window: \"{revitWindow.Name}\"");
            sb.AppendLine($"  Bounds: {revitWindow.BoundingRectangle}");
            sb.AppendLine($"  ControlType: {revitWindow.ControlType}");
            sb.AppendLine($"  AutomationId: \"{revitWindow.AutomationId}\"");
            sb.AppendLine();

            var lines = new List<string>();
            DumpTree(revitWindow, 0, depth, lines, filterType, filterId);
        ...
        var treeDepth = Program.Verbosity == "full" ? depth : Program.Verbosity == "normal" ? Math.Min(depth, 2) : 0;
        var root = OutputFormatter.FromAutomationElement(revitWindow, 0, treeDepth);
        ...
        var result = new CommandResult
        {
            ...
            Success = true,
            Data = Program.Verbosity == "minimal"
                ? new { name = revitWindow.Name, outputFile }
                : new { name = revitWindow.Name, tree = root, depth, filterType, filterId, outputFile }
        };
        if (Program.IsScreenshot)
            result.Screenshot = ScreenshotHelper.CaptureWindow(revitWindow);
        Console.Write(OutputFormatter.FormatResult(result, Program.IsPretty));
...
public class InspectCommand : ICommand
{
    ...
    public Task<int> ExecuteAsync(AutomationElement revitWindow, string[] args)
    {
        ...
        {
            var info = OutputFormatter.FromAutomationElement(revitWindow, 0, 1);
            var result = new CommandResult
            {
                Command = "inspect",
                Success = true,
                Data = Program.Verbosity == "minimal" ? new { name = revitWindow.Name } : new { element = info }
            };
            if (Program.IsScreenshot)
                result.Screenshot = ScreenshotHelper.CaptureWindow(revitWindow);
            Console.Write(OutputFormatter.FormatResult(result, Program.IsPretty));
            ...
            var result = new CommandResult
            {
                ...
                Data = Program.Verbosity == "minimal"
                    ...
                    : new
                    {
                        ...
                        children = Program.Verbosity == "full"
